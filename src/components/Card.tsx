...
export default function CardComponent({ tag, title, description, meta, href }: CardProps) {
  const cta = tag === "Job" ? enDict.card.viewJob : 
              tag === "Event" ? enDict.card.learnMore : 
              enDict.card.explore;

  return (
    <a href={href} className="group glass rounded-[20px] p-[40px_36px] transition-all duration-300 relative overflow-hidden no-underline block hover:bg-[rgba(78,205,196,0.05)] hover:border-[rgba(78,205,196,0.3)] hover:translate-y-[-6px] hover:shadow-[0_20px_50px_rgba(0,0,0,0.3)]">
